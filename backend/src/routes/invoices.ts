...
function tryWriteBase64FontToTmp(): string | null {
  try {
    const notoModulePath = path.join(__dirname, '..', '..', 'assets', 'fonts', 'noto-base64.js');
    if (!fs.existsSync(notoModulePath)) return null;
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const noto = require(notoModulePath);
    let maybeB64: any = null;
    if (typeof noto === 'string') maybeB64 = noto;
    else if (noto && typeof noto === 'object') maybeB64 = noto.base64 || noto.default || noto.noto || null;
    if (!maybeB64 || typeof maybeB64 !== 'string' || maybeB64.length < 100) return null;
    const b64 = maybeB64.replace(/\r|\n/g, '');
    const tmpDir = path.join(__dirname, '..', '..', 'tmp');
    if (!fs.existsSync(tmpDir)) fs.mkdirSync(tmpDir, { recursive: true });
    const outPath = path.join(tmpDir, `noto-embedded-${Date.now()}.ttf`);
    fs.writeFileSync(outPath, Buffer.from(b64, 'base64'));
    return outPath;
  } catch (e) {
    console.warn('tryWriteBase64FontToTmp failed:', e && (e as Error).message);
    return null;
  }
}

async function generatePdfWithChromium(html: string, fontPath?: string) {
  ...
  if (!fontDataUrl) {
    try {
      const notoModulePath = path.join(__dirname, '..', '..', 'assets', 'fonts', 'noto-base64.js');
      if (fs.existsSync(notoModulePath)) {
      ...
      console.warn('Failed to load noto-base64.js:', e && (e as Error).message);
    }
  }
  ...
  const tryLaunch = async (opts: any) => {
    try {
      ...
      console.warn('puppeteer.launch failed with opts:', !!(opts && opts.executablePath), err && (err as Error).message);
      throw err;
    }
  };

  try {
    if (process.env.PUPPETEER_EXECUTABLE_PATH) baseLaunchOptions.executablePath = process.env.PUPPETEER_EXECUTABLE_PATH;
    browser = await tryLaunch(baseLaunchOptions);
  } catch (firstErr) {
    ...
    for (const p of commonPaths) {
      if (fs.existsSync(p)) {
        try {
          ...
          browser = await tryLaunch({ ...baseLaunchOptions, executablePath: p });
          break;
        } catch (e) {
          // continue to next
        }
  ...
  try {
    ...
    try {
      ...
      await page.evaluate(async () => {
        ...
        if (d.fonts && d.fonts.ready) {
          try { await d.fonts.ready; } catch (err) { /* ignore */ }
        }
      });
    } catch (e) {
      // ignore evaluate errors but log
      console.warn('Warning waiting for document.fonts.ready:', e && (e as Error).message);
    }
    ...
    try {
      ...
      if (!fs.existsSync(tmpDir)) fs.mkdirSync(tmpDir, { recursive: true });
      const ts = Date.now();
      fs.writeFileSync(path.join(tmpDir, `invoice-debug-${ts}.html`), fullHtml, 'utf8');
      if (fontPath && fs.existsSync(fontPath)) {
        try { fs.copyFileSync(fontPath, path.join(tmpDir, `embedded-font-${ts}.ttf`)); } catch (e) { /* noop */ }
      }
    } catch (e) {
      console.warn('Failed to write debug artifacts:', e && (e as Error).message);
    }
...
router.post('/generate', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    const subtotal = (order.items || []).reduce((s: number, it: any) => s + ((it.quantity || 0) * (it.unitPrice || 0)), 0);
    const pending = (order.pendingPayments || 0);
    const includePending = !!(order as any).pendingPaid;
    ...
    try {
      let fontPath = process.env.SINHALA_FONT_PATH || path.join(__dirname, '..', '..', 'assets', 'fonts', 'NotoSansSinhala-Regular.ttf');
      console.info('Checking Sinhala font path:', fontPath);
      let fontToRegister = fontPath;
      if (!fs.existsSync(fontToRegister)) {
    ...
    doc.on('end', async () => {
      ...
      const fontPath = process.env.SINHALA_FONT_PATH || path.join(__dirname, '..', '..', 'assets', 'fonts', 'NotoSansSinhala-Regular.ttf');
      const useChromium = process.env.PDF_RENDERER === 'chromium' || process.env.USE_CHROMIUM_PDF === 'true' || (fontPath && fs.existsSync(fontPath));
      console.info('Invoice renderer decision: useChromium=', useChromium, ' fontPath=', fontPath);

      if (useChromium) {
        // Build HTML invoice
        const itemsHtml = (order.items || []).map((it: any) => `<li>${it.productName} - Qty: ${it.quantity} ${it.unit ? '('+it.unit+')' : ''}</li>`).join('');
        ...
        try {
          ...
          const pdfBuf = await generatePdfWithChromium(html, fontPath);
      ...
      console.info('Falling back to PDFKit renderer; registered fonts:', Object.keys((doc as any)._fontFamilies || {}));
...
router.get('/generate-test', async (req, res) => {
  ...
  try {
    ...
    const invoice = new Invoice({ orderId, customerId: order.customer, totalAmount, pendingAmount: pending, paymentMethod, isPersonalized: !!order.personalized, paidPending: includePending });
    await invoice.save();
